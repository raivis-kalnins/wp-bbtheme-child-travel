...
function wpbb_travel_assets() {
    $theme = wp_get_theme();
    wp_enqueue_style( 'wpbb-travel-meta', get_stylesheet_uri(), array( 'wp-theme-style' ), $theme->get( 'Version' ) );
    ...
    if ( ! empty( $data['src/scss/public.scss']['file'] ) ) {
        wp_enqueue_style( 'wpbb-travel-app', get_stylesheet_directory_uri() . '/dist/' . ltrim( $data['src/scss/public.scss']['file'], '/' ), array( 'wpbb-travel-meta' ), $theme->get( 'Version' ) );
...
function wpbb_travel_single_content( $content ) {
    if ( !is_singular('trip') || !in_the_loop() || !is_main_query() ) return $content; $id=get_the_ID(); $image=get_the_post_thumbnail_url($id,'large'); $gallery=function_exists('wp_theme_item_gallery_single_markup')?wp_theme_item_gallery_single_markup($id):'';
