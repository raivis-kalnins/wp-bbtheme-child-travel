<?php
/**
 * WP BBTheme child suite 3.8.11.37 - canonical 1320px section grid + media quality owner.
 */
...
if ( ! function_exists( 'wpbb_child_v137_enqueue' ) ) {
    function wpbb_child_v137_enqueue() {
        ...
        // v137 is the only late DOM/layout owner. Retire v136 so its observer and
        // 1180px shell cannot race the canonical 1320px section grid.
        wp_dequeue_style( 'wpbb-suite-v136' );
        wp_deregister_style( 'wpbb-suite-v136' );
...
if ( ! function_exists( 'wpbb_child_v137_body_class' ) ) {
    function wpbb_child_v137_body_class( $classes ) {
        $classes = array_values( array_filter( (array) $classes, static function( $class ) {
            return 0 !== strpos( (string) $class, 'wpbb-v136' );
        } ) );
        $classes[] = 'wpbb-v137';
